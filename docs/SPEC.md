...
| E9 | `new/list/resume` 命令、`--model/--provider` CLI 参数 | §3.8/§3.12 | M2；注：TUI 内 slash `/connect` `/model` `/effort` `/help` `/exit` 已提前实现（2026-08-14，`/` 触发浮窗 + Tab 补全，见 docs/specs/2026-08-14-slash-commands-design.md）——`/status` `/skills` `/mcp` 仍为 M2 |
