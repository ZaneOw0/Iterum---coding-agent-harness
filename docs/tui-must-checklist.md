...
|---|---|---|---|---|---|
| 1 | TUI 以 **session transcript** 为主界面 | ✅ | `packages/tui/src/App.tsx`：主列 `flexGrow={1}` 挂 `Transcript`（Composer/Footer 附属）；`packages/tui/src/components/Transcript.tsx` 逐消息渲染 `MessageView` | `packages/tui/test/transcript.test.tsx`：以 Session 渲染，断言 Thought 行与 tool 结果；text part 由 `MessageView.tsx` 渲染、无独立断言 | ✅ |
...
- `bun test`：101 pass / 0 fail（2026-08-14，T28 后全量）。
- 附录 A 与原文 §17 无出入，**无需回填**。
- 结论：10 条 MUST 全部在组件/数据模型层满足；MUST-7 按键绑定因 Windows 平台实机验证偏离原文（Enter 发送 / Ctrl+J 换行，见 MUST-7 行偏离记录）；MUST-6 的 App 级接线（PermissionDialog 挂载 + composer 阻塞）为已知缺口（README 已如实声明）。

## 疑虑

① MUST-7 的 multiline 按键行为现已有测试锁定（`composer.test.tsx` stdin.write 模拟）；MUST-8 的 >120 列判断仍无独立自动化测试（依赖 `useStdout` 终端宽度，ink-testing-library 未覆盖）；后续建议补 `App` 级渲染测试（含 wide/narrow 两种宽度）。
