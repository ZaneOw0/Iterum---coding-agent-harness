...
> 注：TUI 渲染层组件全部实现并有测试；T22 已完成 `<App>` 挂载与事件流接线（`packages/cli/src/tui.tsx` `runTui` 渲染 `<App>`，Composer 提交经 `driveSession` 驱动 AgentLoop 事件流；见 README"运行"与"已知限制"）。但 `<App>` 尚未挂载 DialogHost（Permission 弹窗未接入，见 MUST-6）；因此 MUST-6 的"通过"指**组件/数据模型层面满足基线要求**，接线缺口已如实标注。
...
- 结论：10 条 MUST 全部在组件/数据模型层满足；MUST-6 的 App 级接线（DialogHost 挂载 + composer 阻塞）为 M1 已知缺口（Permission 弹窗接线为后续任务，README 已如实声明）。
